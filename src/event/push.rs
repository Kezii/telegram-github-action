...
fn format_commit_message(
    ...
    let repo_simple_name =
        &repo.full_name[repo.full_name.find("/").unwrap() + 1..repo.full_name.len()];
...
#[cfg(test)]
mod tests {
    use crate::event::push::{format_commit_message, Commit};
    ...
    pub fn test_commit_message() {
        ...
        let commits = vec![Commit {
            ...
            comment: "commit_comment".to_string(),
        }];
        let result =
            format_commit_message("kviring".to_string(), repo, target.to_string(), commits);

        println!("{}", result);
        assert_eq!(
            result,
            "[kviring](https://github.com/kviring) push to [repo](repo_url):[my\\-branch]\
